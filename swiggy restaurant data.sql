...
#HOW MANY RESTAURANTS HAVE THE WORD "Biryani" IN THEIR NAME?
select count(distinct restaurant_name) as restaurant_with_pizza from swiggy
where restaurant_name like '%Biryani%';

#WHAT IS THE MOST COMMON CUISINE AMONG THE RESTAURANTS IN THE DATASET?
select cuisine,count(*) as common_cuisine from swiggy
group by cuisine
order by common_cuisine desc
limit 1;

#WHAT IS THE AVERAGE RATING OF RESTAURANTS IN EACH CITY?
select city, avg(rating) as average_restaurant_rating from swiggy 
group by city;

#WHAT IS THE HIGHEST PRICE OF ITEM UNDER THE 'RECOMMENDED' MENU CATEGORY FOR EACH RESTAURANT?
select distinct restaurant_name, menu_category, max(price) as highest_item_price from swiggy 
where menu_category='Recommended'
group by restaurant_name, menu_category;

# top 5 most expensive restaurants that offer cuisine other than indian cuisine
select distinct restaurant_name, cost_per_person as expensive_restaurants from swiggy 
